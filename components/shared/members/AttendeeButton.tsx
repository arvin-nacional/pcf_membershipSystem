...
import { deleteAttendee } from "@/lib/actions/attendee.action";

interface Props {
  attendeeId: string;
}

const AttendeeButton = ({ attendeeId }: Props) => {
    ...
    <Menubar className="relative border-none bg-transparent shadow-none">
      <MenubarMenu>
        ...
        <MenubarContent className="absolute right-[-3rem] rounded border bg-light-900 dark:border-dark-400  dark:bg-dark-300 max-sm:w-[50px]">
          {/* item.value === "view" && router.push(`/members/${attendeeId}`);
                item.value === "delete" && deleteMember({ path, attendeeId }); */}
          <MenubarItem
            onClick={() => {
              router.push(`/attendees/edit/${attendeeId}`);
            }}
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction variant="destructive">
                  <Button
                    className="bg-red text-light-800"
                    onClick={() => {
                      deleteAttendee({ path, attendeeId });
                      return toast({
