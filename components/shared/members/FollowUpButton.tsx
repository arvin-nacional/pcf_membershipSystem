...
import { deleteFollowUp } from "@/lib/actions/followup.action";

interface Props {
  id: string;
}

const FollowUpButton = ({ id }: Props) => {
    ...
    <Menubar className="relative border-none bg-transparent shadow-none">
      <MenubarMenu>
        ...
        <MenubarContent className="absolute right-[-3rem] rounded border bg-light-900 dark:border-dark-400  dark:bg-dark-300 max-sm:w-[50px]">
          {/* item.value === "view" && router.push(`/members/${id}`);
                item.value === "delete" && deleteMember({ path, id }); */}
          <MenubarItem
            onClick={() => {
              router.push(`/follow-ups/edit/${id}`);
            }}
